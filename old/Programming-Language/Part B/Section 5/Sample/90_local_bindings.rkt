...
; * let: bind any number of local variables, evaluated in the environment before the let-expression
; * let*: like the ML's let, evaluated in the environment produced from the previous bindings
; * letrec: evaluated in the environment that includes all the bindings
; * define: for defining local variables, same semantics as letrec

; let evaluates all expressions using outer environment,
; *not* earlier bindings
(define (double1 x) ; assume x is 4
  (let ([x (+ x 3)] ; x is 7
        [y (+ x 2)]) ; x is 4, y is 6
    (+ x y -5))) ; x is 7, y is 6, result is 8

; let* is like ML's let: environment includes previous bindings
(define (double2 x) ; assume x is 4
  (let* ( [x (+ x 3)] ; x is 7
          [y (+ x 2)]) ; x is 7, y is 9
    (+ x y -8))) ; x is 7, y is 9, result is 8
...
(define (triple x) ; assume x is 4
  (letrec ( [y (+ x 2)] ; y is 6
            [f (lambda (z) (+ z y w x))] ; f is a function, return z + y:6 + w + x:4
            [w (+ x 7)]) ; w is 11
    (f -9))) ; result is z:-9 + y:6 + w:11 + x:4 = 12
