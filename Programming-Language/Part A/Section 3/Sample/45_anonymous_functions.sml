...
(* Anonymous Functions: an /expression/ form for anonymous functions
 * Syntax:
    * `fn` not `fun`
    * `=>` not `=`
    * No funtion name, just an argument pattern
 * Cannot use for a recursive function
 * only need to use a function in one place
**)
